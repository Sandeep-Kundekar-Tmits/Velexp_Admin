...
import { DELETE_RETAIL_RATE_DATA, GET_RETAIL_RATE_DATA, UPDATE_RETAIL_RATE_DATA, UPLOAD_RETAIL_RATE_DATA } from "../../../api"
...
import UpdateRetailRateData from "./UpdateRetailRateData"
...
const RetailDataRate = () => {
  ...
  const { apifunc: GetRetailRateData, data: RetailrateDataInfo, loading: RetailRateDataLoading } = useGetApiCall()
  // defining the put (update) asus api
  const { apifunc: UpdateRetailRateDataFunc, loading: UpdateLoading } = usePutApiCall("Updated Sucussessfully", () => setSelectedTitle(""))
  // defining the delete asus list
  const { apifunc: DeleteRetailInfo, loading: DeleteLoading } = useDeleteApiCall()
  ...
  const onSave = async (data) => {
    ...
    let UpdatedData = await UpdateRetailRateDataFunc(`${UPDATE_RETAIL_RATE_DATA}${selectedInfo?.id}/`, data)
    if (UpdatedData) {
      setRateData((prev) => {
        return prev.map((ele) => {
  ...
  const onDelete = async () => {
    ...
    let isDeleted = await DeleteRetailInfo(`${DELETE_RETAIL_RATE_DATA}${selectedInfo?.id}/`)
    if (isDeleted) {
      setSelectedTitle("")
  ...
  useEffect(() => {
    GetRetailRateData(GET_RETAIL_RATE_DATA)
  }, [])

  useEffect(() => {
    if (RetailrateDataInfo) {
      setRateData(RetailrateDataInfo)
    }
  ...
  const DownloadRetailData = () => {
    ...
    downloadExcel(exportData, 'Retail_Rate_data.xlsx');
  };

  //  onchange function on the upload file
  const UploadRetailOnChange = async (e) => {
  ...
  // upload function
  const RetailBulkUpoad = async () => {
    // calling the asus bulk upload api
    const uploaded = await BulkAsusUploadFunc(UPLOAD_RETAIL_RATE_DATA, UploadData)
  ...
  return (
    ...
    <div className="p-0">
      ...
      <div>
        {
          (isAdmin && RateData?.length >= 1) && <div className="d-flex align-items-center">
            <FormGroup style={{ marginTop: "10px" }}>
              ...
              <Input type="file" id="gstUpload" onChange={UploadRetailOnChange} />
              {
            ...
            {
              UploadData.length >= 1 && <Button color="primary" onClick={RetailBulkUpoad} className="ms-5 d-flex justify-content-center align-items-center" style={{ height: "2.2rem", width: "5rem" }} >
                {
      ...
      <div className=''>
        ...
        <div className='mt-3'>
          {
            RetailRateDataLoading ? <div style={{ height: "40vh" }} className="container-fluid  d-flex flex-column justify-content-center align-items-center">
              <GridLoader size={20} />
              ...
              <TableContainer
                ...
                onDownloadExcle={DownloadRetailData}
