...
import { GET_ALL_FRNCHISE_RATE_DATA, UPLOAD_FRANCHISE_RATE_DATA_UPLOAD } from "../../../api";
...
const FranchiseRateData = () => {
    ...
    const [AllFranchiseRateDatas, setAllFranchiseRateDatas] = useState([])
    ...
    //  defining the api to get all franchise Rate datas
    const { apifunc: GetAllFranchiseRateDatas, data: AllFranchiseApiRateData, loading: AllFranchiseRateDataLoading } = useGetApiCall()
    // defining the api to upload the retail rate data
    const { apifunc: UploadFranchiseRateData, data: UploadedFranchiseRateData, loading: UploadFranchiseRateDataLoading } = usePostApiCall()
    useEffect(() => {
        //  calling the get all franchise rate data api
        GetAllFranchiseRateDatas(GET_ALL_FRNCHISE_RATE_DATA)
    }, [])

    const UploadRateData = async () => {
        let isUploaded = await UploadFranchiseRateData(UPLOAD_FRANCHISE_RATE_DATA_UPLOAD, selecetedExcelData);
    ...
    useEffect(() => {
        if (AllFranchiseApiRateData) {
            setAllFranchiseRateDatas(AllFranchiseApiRateData)
        }
    }, [AllFranchiseApiRateData])
    ...
    const converToExcel = (type) => {
        ...
        ConvertoEmptyExcel(RequiredList, "Franchise_Rate_data_template",);
    };
    ...
    return (
        <div className='page-content'>
            <div className="container-fluid">
                {/*  header */}
                <MainHeaderComp title="Franchise Rate Data"
                    extraFields={<div className="d-flex justify-content-between align-items-center">
                        <div className="d-flex flex-column me-1">
                            ...
                            <span className="text-black" style={{ fontSize: "12px" }}>Template</span>
                        </div>
                        {["Default "]?.map((ele) => (
                ...
                <Row className="  d-flex align-align-items-center my-3">
                    ...
                    <Col md={4}>
                        ...
                        >
                            {UploadFranchiseRateDataLoading ? "Uploading..." : "Upload"}
                        </Button>
