...
const buildRows = (list) =>
    list.map(i => ([
        ...
        // rto
        i["rto"], pct(i["rto_percent"]),
        //disposed
        i["disposed"], pct(i["disposed_percent"]),
        //lost
        i["lost"], pct(i["lost_percent"]),
        // wab
...
const buildTotalRow = (list) => {
    ...
    return [
        ...
        // delivered
        sum(list, "delivered"),
        calcPercent(sum(list, "delivered"), grandTotal),
        //undelivered
        sum(list, "undelivered"),
        calcPercent(sum(list, "undelivered"), grandTotal),
        // rto
        sum(list, "rto"),
        calcPercent(sum(list, "rto"), grandTotal),
        // disposed
        sum(list, "disposed"),
        calcPercent(sum(list, "disposed"), grandTotal),
        //lost
        sum(list, "lost"),
        calcPercent(sum(list, "lost"), grandTotal),
        //wab
        sum(list, "wab"),
        calcPercent(sum(list, "wab"), grandTotal),
        grandTotal
    ];
};
...
export const useNewOperationDelivaryStrikeRate = () => {
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);

    const downloadExcel = useCallback(
        (data, title = "", extraSheet, meta = {}, carts = {}) => {
            setLoading(true);
            setError(null);
            ...
            setTimeout(() => {
                try {
                    ...
                    const titleRowIndex = metaRows.length + 1;

                    ws["!merges"] = [
                        {
                            s: { r: titleRowIndex, c: 0 },
                            e: { r: titleRowIndex, c: header.length - 1 }
                        }
                    ];
                    ...
                    ws["!cols"] = [
                        { wch: 18 },
                        ...Array(header.length - 1).fill({ wch: 10 })
                    ];
